...
REPO2="https://raw.githubusercontent.com/GeKaStore/WZSC/main/"
function CEKIP () {
MYIP=$(curl -sS ipv4.icanhazip.com)
IPVPS=$(curl -sS https://raw.githubusercontent.com/GeKaStore/vps_access/main/ip | grep $MYIP | awk '{print $4}')
if [[ $MYIP == $IPVPS ]]; then
domain
Pasang
else
...
echo -e "$green                $NC"                                                                                                                          
echo -e "$green♥ TERIMAKSIH TELAH MEMAKAI SCRIPT WUZZSTORE ♥$NC"
sleep 2
sleep 5
echo -e  "${green}┌──────────────────────────────────────────┐${NC}"
...
function key2(){
    ...
    clear
    echo -e "${green}┌──────────────────────────────────────────┐${NC}"
    echo -e "${green}│ \033[1;37m    IZIN LISENSI              ${green}│${NC}"
    echo -e "${green}└──────────────────────────────────────────┘${NC}"
        MYIP=$(curl -sS ipv4.icanhazip.com)
        ...
        curl -s https://pastebin.com/raw/3hrmp51f > /etc/github/email
        curl -s https://pastebin.com/raw/XkuY5wwt > /etc/github/username
        clear
        ...
        cd
        git clone https://github.com/GeKaStore/vps_access >/dev/null 2>&1
		    cd vps_access
		    sed -i "/# ADMIN/a ### ${author} ${hhari} ${MYIP} @VIP" /root/vps_access/ipx
        sed -i "/# SSHWS/a ### ${author} ${hhari} ${MYIP} ON SSHWS @VIP" /root/vps_access/ip
        sleep 1
        ...
        git remote add origin https://github.com/${USERGIT}/vps_access >/dev/null 2>&1
        git push -f https://${APIGIT}@github.com/${USERGIT}/vps_access >/dev/null 2>&1
        sleep 1
        cd
        rm -rf /root/vps_access
        clear
}
function domain(){
fun_bar() {
...
res2() {
wget ${REPO2}install/ssh-vpn.sh >/dev/null 2>&1
...
res7() {
wget ${REPO2}menu/update.sh >/dev/null 2>&1
